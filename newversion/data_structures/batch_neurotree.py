import torch
import functools
from constant.common_keywords import \
...
class BatchNeuroTree(object):
    ...
    def getVisitCount(self):
        return [_.__dict__[VISIT_COUNT_KEY] for _ in self.nodes]


    @functools.lru_cache(maxsize=3)
    def getChildCount(self):
        return [_.__dict__[CHILD_COUNT_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getChildAdjacencyMatrix(self):
        return [_.__dict__[CHILD_ADJACENCY_MATRIX_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getIndices(self):
        return [_.__dict__[INDICES_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getX(self):
        return [_.__dict__[X_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getY(self):
        return [_.__dict__[Y_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getDeconvX(self):
        return [_.__dict__[DECONV_X_KEY] for _ in self.nodes]


    @functools.lru_cache(maxsize=3)
    def getHiddens(self):
        return [_.__dict__[HIDDEN_KEY] for _ in self.nodes]

    @functools.lru_cache(maxsize=3)
    def getDeconvHiddens(self):
        return [_.__dict__[DECONV_HIDDEN_KEY] for _ in self.nodes]


    @functools.lru_cache(maxsize=3)
    def getPredicts(self):
        return [_.__dict__[PREDICT_KEY] for _ in self.nodes]


    @functools.lru_cache(maxsize=3)
    def getSubTasks(self):
        return [_.__dict__[SUBTASK_KEY] for _ in self.nodes]

    # @functools.lru_cache(maxsize=1)
    def getSubtask(self, i):
        return self.nodes[i].__dict__[SUBTASK_KEY]
...
########## Version 1 ##########
    ...
    def setHiddens(self, batch_item):
        for i in range(len(self.nodes)):
            self.set_i(HIDDEN_KEY, batch_item[i], i)

    def setDeconvHiddens(self, batch_item):
        for i in range(len(self.nodes)):
            self.set_i(DECONV_HIDDEN_KEY, batch_item[i], i)

    def setDeconvXs(self, batch_item):
        for i in range(len(self.nodes)):
            self.set_i(DECONV_X_KEY, batch_item[i], i)


    def setIndices(self, batch_item):
        for i in range(len(self.nodes)):
            self.set_i(INDICES_KEY, batch_item[i], i)

    def setPredicts(self, batch_item):
        for i in range(len(self.nodes)):
            self.set_i(PREDICT_KEY, batch_item[i], i)
    ...
    def get_child_count(self):
        return max(self.get(CHILD_COUNT_KEY))
