...
from models.encoders.recursive_encoder import RecurrentNeuralNetwork
    ...
    def __init__(self, input_dim, hidden_dim,
        ...
        if version == 'lan':
            for i in range(max_lv):
                self.__setattr__(f'rnn{i}', RecurrentNeuralNetwork(self.input_dim_with_bias, hidden_dim))
                self.__setattr__(f'gnn{i}', GraphConvolutionalLayer(self.hidden_dim))
            ...
            for i in range(max_lv):
                ...
                self.__setattr__(f'rnn{i}', RecurrentNeuralNetwork(self.input_dim_with_bias, hidden_dim))
                self.__setattr__(f'gnn{i}', GraphAttentionLayer(self.hidden_dim))
                rnn.append(self.__getattr__(f'rnn{i}'))
                gnn.append(self.__getattr__(f'gnn{i}'))
                # self.__dict__[f'rnn{i}'] = RecurrentNeuralNetwork(self.input_dim_with_bias, hidden_dim)
